...
/// <summary>
/// <c>nightshift add &lt;order.json&gt; [--sha]</c> — a one-shot seed/reconcile. Projects the work order into
...
internal static class AddCommand
{
    ...
    {
        string? path = OrderFile.FirstPositional(args);
        ...
        Console.WriteLine($"seeded order {order.OrderId} @ {OrderFile.ShortSha(sha)}: {r.SpecsCreated} spec(s) created, {r.Added} ready added, {r.Removed} removed");
        return 0;
    }
}
