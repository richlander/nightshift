...
**Database ownership guards the file; a second, independent ownership guards the socket endpoint (#212).** The `ModeLock` keys on the database, not the Unix socket a daemon binds — and those are separable. A second daemon started on the *same* `--socket` but a *different* `--db` takes a *different* `ModeLock` (no conflict), and under the old startup it would then unconditionally `unlink(2)` the first daemon's socket and bind its own. Unix lets a listening socket be unlinked without disturbing the connections already on it, so the two daemons would silently split coordination state: existing clients stay wired to daemon A and its database, while every new client reaches daemon B — exactly the split `ModeLock` exists to prevent, but one file-ownership can't see. Startup closes this with two mechanisms, both applied **before** the socket path is bound, and **neither of which ever deletes a socket path**:

- **An exclusive `SocketLock` on the socket's canonical identity.** It reuses the same `flock` primitive as the database lock (both are now the shared `FileLock`: one non-blocking, close-on-exec, OS-released advisory lock), only on a `<socket>-socklock` sidecar and raising a typed `TurnstileSocketInUseException`. Two compliant daemons that would bind the same endpoint — even through different path spellings, since the sidecar is keyed on the socket's `realpath` identity via the same `CanonicalPath` helper the database uses — contend on this one lock, so the second fails fast. This both serializes daemon startup and closes the absent-path race (two daemons starting on a fresh socket that both see no file). The socket owner is exclusive (there is no shared-reader analogue for binding a listener).
- **A fail-closed existence check.** The lock alone cannot protect an *older* daemon built before the lock existed, or any foreign listener that holds no socklock — such a process participates in no lock, so a new daemon would acquire the free lock over a live endpoint. And a daemon **cannot prove a socket path is stale** without cooperation from whatever bound it: on macOS a saturated but live listener can refuse a `connect(2)`, so no connect-only probe can tell a crash leftover apart from a busy live server (and on Linux a saturated accept queue can block the probe indefinitely). So once the lock is held, if **any** filesystem entry exists at the requested or canonical path — a live listener, a stale socket, a regular file, or a symlink (including a dangling one) — startup **refuses** with `TurnstileSocketInUseException` and never unlinks it. Safety wins over crash-restart convenience: Turnstile will not silently delete a path that might be live. There is deliberately no `--force`, no automatic retry, and no polling.

Both locks are non-blocking, so their fixed order — **socket endpoint first, then database** — cannot deadlock; it is a documented policy, not a hazard. Both are held for the daemon's whole lifetime and released on any startup failure and after shutdown (or by the OS on a crash). The net contract: two daemons on the same socket but different databases no longer split — the second is refused; two daemons on the same database but different sockets remain governed by `ModeLock` as before; an **ordinary graceful restart still succeeds**, because a graceful shutdown removes the daemon's own socket (Kestrel unlinks it on unbind) so the next start finds the path free; an **unclean crash** leaves the socket file behind as an explicit operator-cleanup condition — verify nothing is listening, remove the path, and retry — the deliberate tradeoff for never unlinking a path that might be live; if an external process races to bind after the existence check, that surfaces as a visible Kestrel bind failure and still deletes nothing; and every conflict is a `TurnstileUnavailableException`, surfacing through `turnstile serve` as the unavailable exit code and a first-line `turnstile:` error, no stack trace. As with the database lock, this is coordination correctness under the same supported-symlink contract, not security containment against a hostile hardlink or a post-resolution path swap.
