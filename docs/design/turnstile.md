...
- **Deadlines are stored in whole server seconds, truncated.** `expires_at = floor(now) + ttl`, where `floor(now)` drops the fraction of the current second already elapsed at grant time. So a lease requested for `ttl` seconds is actually live for somewhere in **`(ttl − 1, ttl]`** — uniformly, depending only on where in the second the grant lands. `ttl_remaining` and the reported `ttl` echo the *requested* value, not this effective lifetime. This is a deliberate, cron-style truncation, not a rounding bug; it is documented here because the shortfall is unbounded below in *relative* terms and callers must plan for it. A round-up or higher-resolution deadline is reserved for a future caller that genuinely needs a sub-ten-second TTL.
- **Renew against `ttl − 1`, not `ttl`.** A holder that derives its keepalive cadence from the nominal TTL — renew every `ttl/2`, the obvious choice — is computing against a number up to a full second larger than the lease it actually holds. Harmless at `ttl = 2700`, fatal at `ttl = 2`, where the effective lifetime can be a hair over zero. Budget the missing second: pick TTLs comfortably above your renewal interval (tens of seconds or more), and treat any TTL near 1 as already expired.
