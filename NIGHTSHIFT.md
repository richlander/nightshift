...
(or coordinator) to import at the start of a shift and use to convert this repo's issues into **orders**
that workers execute. Read your role's skill for *how* to plan and run a shift; this charter says only
...
The engineering bar every order is built and reviewed against is in [`AGENTS.md`](AGENTS.md).
...
- **Set each order's `standard`** to the design note it must conform to, or inherit the plan's when they
  share one.

Everything else — one order = one landable PR, `paths` kept disjoint across parallel orders, `after`
gating on merge, the two-clean-review gate — is standard Nightshift mechanics your skill owns.
