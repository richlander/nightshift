...
/// <summary>
/// The single-writer revision counter must publish only committed revisions, and it allocates from the
/// durable <c>meta.committed_revision</c> read under BEGIN IMMEDIATE — never a cached value. A revision
/// allocated by an in-flight transaction is transaction-local: no reader sees it, and a rolled-back
/// transaction reuses its allocated numbers for the next commit, so a resume cursor can never skip a reused,
/// committed event (#192 / #199).
/// </summary>
public sealed class WriteActorTests : IDisposable
{
    ...
    /// <summary>Reads the durable counter on a fresh connection — safe from any thread while the writer holds
    /// its own connection, and the WAL snapshot never blocks on the writer's lock.</summary>
    ...
    public async Task InFlightAllocatedRevision_IsNotPublished_AndRollbackReusesIt()
    {
        ...
        using var writer = new WriteActor(_conn, onCommitted: () => Interlocked.Increment(ref committedNotifications));
        ...
        Assert.Equal(0, MetaRevision());            // unadvanced after rollback
        Assert.Equal(0, committedNotifications);     // a rolled-back write notifies no watcher
        Assert.Equal(0, MetaRevision());        // the durable counter rolled back with the row
        ...
        Assert.Equal(1, idB);
        Assert.Equal(1, MetaRevision());            // published only now, post-commit
        Assert.Equal(1, committedNotifications);
        Assert.Equal(1, MetaRevision());        // durable counter advanced atomically with the row
